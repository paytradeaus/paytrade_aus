...
- **Missing required field is the same failure class:** a non-nullable `@Field()` (no `nullable: true`) omitted from the FE payload also dies as `BAD_USER_INPUT` before the resolver — zero backend logs, Save button looks dead. `AddPaymentInput.input_date` is required; every Other Payment form must send `input_date: new Date().toISOString()`. Both overpayment-refund forms omitted it and silently could never save. When one form of a family fails and siblings work, diff their payload builders field-by-field against the DTO.
