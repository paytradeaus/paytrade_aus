...
/**
 * Task #161 — AI Billing, Credits & Stripe Top-up.
 *
 * Creates the billing schema:
 *   - `ai_credit_ledger`         (immutable per-event ledger w/ before/after balance)
 *   - `ai_credit_balances`       (per-business cached balance; rebuildable from ledger)
 *   - `ai_billing_settings`      (per-business auto top-up + payment method ref)
 *   - `ai_credit_purchases`      (admin-visible Stripe top-up history)
 *
 * Extends:
 *   - `subscription_plan_details.monthly_ai_credit` (numeric, dollars)
 *
 * All operations are guarded with IF NOT EXISTS / DO blocks so they apply
 * cleanly to both dev (synchronize) and the existing production DB without
 * disturbing existing rows.
 */
export class AiBillingFoundation1715731600000 implements MigrationInterface {
  name = 'AiBillingFoundation1715731600000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // --- subscription_plan_details.monthly_ai_credit ----------------------
    // Synchronize-managed table is named `subscription_plan_details`.
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP TABLE IF EXISTS "ai_credit_purchases";`);
    await queryRunner.query(
      `DROP TYPE IF EXISTS "ai_credit_purchases_status_enum";`,
    );
    await queryRunner.query(
      `DROP TYPE IF EXISTS "ai_credit_purchases_trigger_enum";`,
    );
    await queryRunner.query(`DROP TABLE IF EXISTS "ai_billing_settings";`);
    await queryRunner.query(`DROP TABLE IF EXISTS "ai_credit_balances";`);
    await queryRunner.query(`DROP TABLE IF EXISTS "ai_credit_ledger";`);
    await queryRunner.query(
      `DROP TYPE IF EXISTS "ai_credit_ledger_event_type_enum";`,
    );
    await queryRunner.query(`
      DO $$
      BEGIN
        IF EXISTS (
          ...
          ALTER TABLE "subscription_plan_details" DROP COLUMN "monthly_ai_credit";
        END IF;
      END $$;
    `);
  }
