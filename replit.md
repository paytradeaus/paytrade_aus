...
- **Route Pre-warming**: `start-production.sh` warms up 20 key routes (public, user, admin) after frontend startup by hitting each route to force Node.js to load and cache the route modules in memory. If any route returns 500, it retries after 3s, and if still failing, restarts the frontend process. Up to 3 warmup attempts before continuing.
