...
- **Database Backups**: `DatabaseBackupModule` runs `pg_dump | gzip` every 6 hours (cron `0 */6 * * *`) in production, uploading compressed `.sql.gz` files to Cloudflare R2 under `database-backups/`. Tiered retention: keep 30 most recent backups, then 1 per day for 30 days, then 1 per month for 6 months. Worst-case data loss: 6 hours. Combined with Railway snapshots for defence-in-depth. Service also exposes `listBackups()` and `downloadBackup()` methods.
- **Startup Seeders**: `OnApplicationBootstrap` seeders run on every deploy. `AdminMenuSeederModule` seeds admin menus. `ComplianceSeederModule` seeds `compliance_checks`, `pta_compliances`, `rta_compliances`, and `compliance_settings` from JSON data files in `back-end/src/libs/@seeders/compliance-seed-data/`. Insert-if-missing logic: existing rows (matched by ID) are never overwritten, so admin edits are preserved.
