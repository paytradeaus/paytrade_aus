...
- **Deployment Stability**: Production uses `npx next start` (NOT standalone mode — standalone causes EIO filesystem errors on Replit). No `--max-old-space-size` memory caps (let Node.js manage its own GC). `no-store, no-cache` headers for HTML and RSC responses to mitigate stale cache issues with Next.js Server Actions. Both backend and frontend have auto-restart loops in `start-production.sh`. The proxy sets `host: localhost:5001` header for frontend requests to prevent Next.js host header validation issues. Frontend GET/HEAD requests have automatic retry (up to 3 retries with 1s delay) for 500 errors to handle transient EIO filesystem errors — with hop-by-hop header sanitization and client abort cleanup.
- **Route Pre-warming**: `start-production.sh` warms up 20 key routes after every frontend start (initial and restarts). If any route returns 500, retries after 3s, and if still failing, kills and restarts the frontend. Up to 3 warmup attempts per start cycle.
- **EIO Auto-Restart**: `production-server.js` tracks exhausted-retry 500 errors via `trackEioError()`. If 3+ errors occur within 60s, auto-kills the `next start` process (2-minute cooldown between restarts). The bash restart loop in `start-production.sh` picks it back up with full warmup. Also triggered by health check failures.
- **Loading Retry Page**: When a page request gets a 500 after all proxy retries are exhausted, users see the standard PayTrade loading screen (circular spinner with logo) with "Reconnecting..." message and auto-retry (6 attempts with escalating delays: 5s, 5s, 8s, 10s, 15s, 20s). On success the page auto-refreshes. RSC requests (`_rsc`) return a plain 500 to trigger Next.js client-side error handling. The maintenance page (wrench icon, "We'll be right back") is reserved for when the entire frontend is unresponsive. A once-daily `[INFO]` email alert (amber header) is sent the first time the loading page is served each day, including the count of loading pages served.
- **Monitoring**: `/health` endpoint verifies database connectivity (runs `SELECT 1`) and returns 200 with `checks: { database: 'ok' }` when healthy, or 503 with `checks: { database: 'unhealthy' }` when degraded. `production-server.js` monitors both backend (port 3001) and frontend (port 5001) health every 30 seconds, parses health response body to log specific failing checks, and sends Brevo email alerts on failures. `KeepAliveService` pings `/health` every 5 minutes and logs degraded status details.
