...
- [Retention bill inbound import — verification](docs/architecture/xero-retention-bill-import-verification.md) — executable per-scenario verification for 3-line ACCPAY retention bills (harness output committed). Standard shapes (ex-GST/inc-GST/ACCREC/PAID) verified clean. One real bug: offsetting retention sub-lines over-counted by the `Math.abs` reducer.
...
- [Xero trust-movements two-way sync](docs/architecture/xero-trust-movements-sync.md) — Task #231: outbound `PT-MOV-{id}` BankTransfer push for Withdrawal / Top Up / Interest / Bank Charge / Top Up Retention payments between trust accounts and their `associated_cash_account_id`, anti-echo inbound matcher, reversal, and `trust_movement` manual re-sync.
...
- [Stripe subscription GST mode](docs/architecture/stripe-subscription-gst-mode.md) — `subscription_details.is_gst_inclusive` flag + `LEGACY_INCLUSIVE_GST_SUB_IDS` env: Signature stays inclusive ($300 = $272.73 + $27.27 GST), everyone else is exclusive (10% on top via Stripe TaxRate). Do NOT enable Stripe Tax in the dashboard — we use manual TaxRates only.
