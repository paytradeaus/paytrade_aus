...
| 60 | PTA Compliance (ALBA) | Check #6 "Payments to Subcontractors" — sub-rule failed (1 of 4) | Open Compliance → ALBA → PTA → "Payments to Subcontractors". Live code (`paymentsToSubcontractors` in `pta-functions.ts`) treats a PTA payment as compliant when its status is **Confirmed (Paid)** OR **Matched** to a bank transaction. A sub-rule fails when (a) at least one PTA payment is still `Unconfirmed - Unmatched`, or (b) one or more Supplier Payment Schedule Notices are in `Not Sent` / `Draft`. The 5 "Paid - Unmatched" claims on ALBA (#100068, #100065, #100066, #100051, #100067) keep their underlying payments in `Unconfirmed - Unmatched`, which trips sub-rule 2. | Either confirm each PTA payment as Paid or match it to a bank transaction in Reconcile, and send any draft/unsent Supplier Payment Schedule Notices. Use the Reconcile workflow weekly and clear the "Notices to do" tile. |
| 61 | PTA Compliance (ALBA) | Check #6 "Payments to Subcontractors" — sub-rule failed (2 of 4) | Same as above | Same as above |
| 62 | PTA Compliance (ALBA) | Check #6 "Payments to Subcontractors" — sub-rule failed (3 of 4) | Same as above | Same as above |
| 63 | PTA Compliance (ALBA) | Check #6 "Payments to Subcontractors" — sub-rule failed (4 of 4) | Same as above | Same as above |
