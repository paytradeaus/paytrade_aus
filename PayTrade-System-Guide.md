...
#### Sync Log Summary
- Shows a summary of recent Xero sync activity for the company
- Visible only when a Xero integration is active
- Displays the total sync log count with a breakdown: Synced (green), Warnings (orange), and Failed (red)
- Lists the 10 most recent sync log entries (all statuses: Succeeded, Warning, Failed)
- Each entry shows the sync type, description, and colour-coded status
- Clicking an entry navigates to the detailed sync log view for that record
- The "View sync log" button links to the Xero Dashboard for full sync log access
...
**Dialog presentation *(Updated 2026-05-15)*:** Manual sync opens as a **full-screen popup** (95vw × 95vh) instead of the older centred modal — both the single-record tab and the catch-up tab need horizontal room for the side-by-side PT ↔ Xero panels and the multi-row catch-up table. The popup uses the system surface colour (`var(--theme-light)`, so it re-themes correctly in dark mode), pins the **Single record / Catch-up by date range** tab switcher to the top as a sticky strip with a 1px shadow under it, and keeps a single body scroller — the Pico header and footer never scroll out of view. Below 768px the popup goes edge-to-edge (100vw × 100vh, square corners) and any inline `.grid` filter rows inside the body collapse to a single column so phone-width screens never overflow horizontally. The chrome around the dialog is also collapsible: the left sidebar and the right AI panel each gained a chevron pill at their column divider, so an operator who needs the full width for a wide catch-up window can hide both rails without leaving the dialog. The popup is reached via a `fullScreenPopup` flag on the shared `BaseModal` component — it is the same control surface as before, just rendered in a wider frame. The catch-up **row-detail** sub-dialog reuses `pt_overviewinfo > .grid` and now reflows that grid with `repeat(auto-fit, minmax(360px, 1fr))` inside the fullscreen popup so its PT-side and Xero-side info panels sit side-by-side instead of stacking under each other in a sea of whitespace.
