...
import { jsonLdText, slugifyString } from "@/utils";
interface Props {
  params: {
    slug: string[];
    category: string;
...
export default async function Page({ params }: Props) {
  const response = await setMetadata({ slugOrId: params?.slug[1] }); // Ensure response is fetched
  return (
    <>
      {response && (
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              description: jsonLdText(response?.blogResource?.content, 200),
      ...
      <Home screen={"HOW-TO-GUIDES-DETAIL"} />
    </>
  );
}
