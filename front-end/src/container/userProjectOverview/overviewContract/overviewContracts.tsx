...
import styles from "./overviewContracts.module.scss";
...
const OverviewContracts = ({ selectedContract }: any) => {
  const { project_id } = selectedContract;
  const [companyId, setCompanyId] = useState<number | null | any>(null);
  const [contracts, setContracts] = useState<ContractType[]>([]);
  const showPagination = contracts?.length > 5;
  ...
  useEffect(() => {
    (async () => {
      const response = await getProjectsLists(CompanyId);
      if (response) {
  ...
  }, [companyId, page, perPage, selectedContract?.project_id, debouncedSearch]);
  ...
  const fetchData = async (page: number, rowsPerPage: number) => {
    setLoading(true);
    try {
      if (selectedContract?.project_id == null) {
        return;
      }
        ...
        let printDataObjCreation = responseData?.contract_list?.map(
          (contract: ContractType) => {
            return {
              ...
              initial_contract_sum: contract?.initial_contract_sum,
              variation_amount: contract?.variation_amount,
            };
          }
        );
  ...
  const handleOptionClick = async (data: {
    ...
    const { id, option, contract_status } = data;
    ...
    if (option === "Edit") {
      ...
      ("Move To Current");
      {
        if (contract_status === "Completed") {
          ...
          await updateContractStatusById({ id, status: "In Progress" });
          // Refresh the contract list after deletion
          fetchData(page, perPage);
        } else if (contract_status === "Deleted") {
          // Move contract to Draft
          await updateContractStatusById({ id, status: "Draft" });
          // Refresh the contract list after deletion
          fetchData(page, perPage);
        }
  ...
  const handleCompletedFunction = async (id: string) => {
    ...
    try {
      ...
      // Refresh the contract list after deletion
      fetchData(page, perPage);
      toast.success("This contract has been archived.");
    } catch (error) {
      console.error("Error deleting user from company:", error);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      format: (row: any) => `$ ${row.initial_contract_sum.toFixed(2)}`,
      wrap: false,
      maxWidth: "200px",
      left: true,
    },
    {
      ...
      format: (row: any) =>
        row.variation_amount !== null
          ? `$ ${row.variation_amount.toFixed(2)}`
          : "",
      wrap: true,
      maxWidth: "160px",
      center: true,
  ...
  const handlePrintPDF = () => {
    ...
    generateAndPrintPDF(formatedTableData, headerNames, "contracts");
  };
  ...
  return (
    <div className={styles.dataContainer}>
      <div className={styles.addButton}>
        <>
          {selectedContract?.project_status === "In Progress" && (
            <FormButton
              className={styles.buttonStyles}
              onClick={() =>
                router.push(
                  `/user/contracts/add?projectid=${selectedContract?.project_id}&projectname=${selectedContract?.project_name}&projectrole=${selectedContract?.project_role}&overview=${selectedContract?.id}`
                )
              }
            >
              + Add
