...
import styles from "./sendEmailTemplate.module.scss";
...
} from "./sendEmailTemplate.functions";
import { ApplicationURLS } from "@/common/applicationURLS";
import CustomEditor from "@/components/editor/editor";
import { InsertActivityLog, multipleFileUploadApi } from "@/app/api/commonAPIs";
...
import { FileUploadResponseData } from "./sendEmailTemplate.types";
...
const SendEmailTemplate = (props: any) => {
  ...
  useEffect(() => {
    if (isView && emailViewData?.id) {
      ...
      let ccOptions = emailViewData?.emailCcIds.map((each: any) => {
        return {
          label: each,
          value: each,
        };
      });
      let toOptions = emailViewData?.toEmails.map((each: any) => {
        return {
          label: each,
          value: each,
        };
      });
      setCcEmailSelectedOpt(ccOptions);
      setToEmailSelectedOpt(toOptions);
    ...
    <div className={styles.mainCon}>
      <ReusableBreadcrumb
        items={[
          ...
          {
            href: ApplicationURLS.ADMIN_COMMUNICATION,
            label: "Communication",
            active: false,
          },
          {
            href: "",
            label: isView ? "View" : "New",
            active: true,
          },
        ]}
        ...
        <div className="w-100">
                ...
                <Row className={`${styles.textFieldStyles}`}>
                  <Col className={styles.eachFieldBottom}>
                    <TextField
                      ...
                      disabled
                      value={
                        isView ? emailViewData?.emailFromId : fromEmailData
                      }
                ...
                <Col>
                  ...
                  <div className={styles.previewBoxStyle}>
                    ...
                    <div className={styles.emailsViewCon}>
                      ...
                      <span className={styles.toStyle}>
                        {formik?.values?.toEmails}
                      </span>
                    </div>
                    <div className={styles.emailsViewCon}>
                    ...
                    <div className={styles.paymentId}>
                      <div
                        dangerouslySetInnerHTML={{
                          __html: isView ? emailViewData?.body : editorContent,
                        }}
                      />
                    </div>
                  </div>
          ...
          <div className={styles.btnContainer}>
            <FormButton
              ...
              onClick={() => router.push(ApplicationURLS.ADMIN_COMMUNICATION)}
...
export default dynamic(() => Promise.resolve(SendEmailTemplate), {
  ssr: false,
});
