...
const ArticlesList = () => {
  ...
  useEffect(() => {
    (async () => {
      const [categoryData, authorsResponse] = await Promise.all([
        AdminfetchAllMasterTypeDetails("Resource Category"),
        AdminListAllBlogResAuthors(),
      ]);
  ...
  const handleOptionClick = async (data: {
    ...
    if (option === "Delete") {
      ...
      setPopupMessage((prev) => ({
        headerMsg: "",
        subHeaderMsg: "Are you sure you wish to delete this resource  guide?",
      }));
    }
    if (option === "status") {
      ...
      setPopupMessage((prev) => ({
        headerMsg: "",
        subHeaderMsg: `Are you sure you wish to ${statusName} this resource  guide?`,
      }));
    }
    if (option === "Edit") {
      router.push(`${ApplicationURLS.ADMIN_ARTICLE_EDIT}/${id}`);
    }
    if (option === "Preview") {
      setCookie("resourceId", id);
      router.push(
        `${ApplicationURLS.RESOURCES_RECOMMENDED}${name
          .replaceAll(" ", "-")
          .toLowerCase()}/${id}`
      );
    }
  ...
  const handleModalPopUpFunction = async () => {
    ...
    if (actionData.option === "Delete") {
      ...
      let response = await AdminUpdateBlogResource(
        payload,
        "Resource guide has been deleted."
      );
      if (response) {
        await getListAllAdminArticles(page, perPage);
      }
    ...
    if (actionData.option === "status") {
      ...
      let response = await AdminUpdateBlogResource(
        payload,
        `Resource guide has been ${actionData?.status}.`
      );
      if (response) {
        await getListAllAdminArticles(page, perPage);
      }
  ...
  function downloadExcel() {
    ...
    convertJsonToExcel(printDocumentData, "Resource list", columnNames);
  }
  const handlePrintPDF = () => {
    ...
    generateAndPrintPDF(formatedTableData, headerNames, "resource", true);
  };
  const columns = [
    {
      name: "Category",
  ...
  return (
    <div className={styles.dataContainer}>
      ...
      <div className={styles.headerAndButtonCon}>
        <span className={styles.headerText}>Resource Guide</span>
        <FormButton
          className={styles.buttonStyles}
          onClick={() => router.push(ApplicationURLS.ADMIN_ARTICLE_ADD)}
        >
          + Add
        </FormButton>
      </div>
