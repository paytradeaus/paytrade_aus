...
export const fetchAllRetentionInPaymentsList = async (
  ...
  try {
    ...
    if (
      response &&
      response.data.fetchAllRetentionInPaymentsList.status === SUCCESS
    ) {
      return response.data.fetchAllRetentionInPaymentsList.data;
    }
    if (
      response &&
      response.data.fetchAllRetentionInPaymentsList.status === ERROR
    ) {
      return null;
    }
  } catch (error: any) {
...
export const fetchRetentionsSummary = async (
  ...
  try {
    ...
    if (response && response.data.fetchRetentionSummary.status === SUCCESS) {
      return response.data.fetchRetentionSummary.data;
    }
    if (response && response.data.fetchRetentionSummary.status === ERROR) {
      toast.error(response.data.fetchRetentionSummary.message);
      return null;
    }
  } catch (error: any) {
    return null;
