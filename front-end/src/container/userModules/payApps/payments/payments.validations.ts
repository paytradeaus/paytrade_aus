...
import { replaceDollarSymbol } from "@/common/commonFunctions";
...
const paymentsSchema = Yup.object().shape(
  {
    ...
    input_date: Yup.string().when(["payment_type"], (otherFieldData: any) => {
      if (
        ...
        return Yup.string().test("input date", function (value, formData: any) {
          const isOnboardingModelOpen = formData.parent.isOnboardingModelOpen;
          ...
          return Yup.string()
            ...
            .test("payless amount", function (value, formData: any) {
              ...
              if (+replaceDollarSymbol(value) <= 0) {
                ...
                return formData.createError({
                  path: formData.path,
                  message: "Payless amount should be lesser than claim amount",
                });
              }
              return true;
            });
