...
import { CardAuthor, CardAuthorCategory } from "../../../../common/constants";
...
const ArticlesCategoryPage = () => {
  ...
  async function getCategoryList() {
    ...
    await fetchBlogsAndResources(postData).then((response: any) => {
      setArticlesCategory(response?.blogResources);
    });
  }
    ...
    <div className={styles.mainContainer}>
      ...
      <div className={styles.homeContainerStyle}>
        <Container>
          ...
          <Row className="mt-4">
            {articlesCategory.map((item: any, i: number) => (
              <Col lg={4} key={i} onClick={() => handleRoute(item)}>
                <div className={styles.cardBlock}>
                  <Cards
                    imgSrc={
                      item?.banner?.file_path || "/assets/image-placeholder.jpg"
                    }
                    title={item?.title || ""}
