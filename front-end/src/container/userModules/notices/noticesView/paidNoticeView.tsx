...
import SignaturePadComponent from "../canvasSignaturePad/signaturePad";
import { getCookie } from "cookies-next";
import { RootState, useAppSelector } from "@/redux/store";

const PaidNoticeView = (props: any) => {
  const { data, isView = false, isAdmin = false, ...rest } = props;
  const [selectedToggled, setSelectedToggled] = useState<string>("Premium");
  ...
  const onHandleSaveDraftClick = async () => {
    setDisabledSaveBtn(true);
    if (!noticeData?.notice_id) {
      setDisabledSaveBtn(false);
      return;
    }
    if (files?.length > 0 || supportFiles.length > 0) await handleFileUploads();
    let payload = {
      notice_id: noticeData?.notice_id,
      status: noticeData?.status === "Not Sent" ? "Draft" : noticeData?.status,
    };
    if (
  ...
  const onSaveSendClick = async () => {
    ...
    if (response) {
      ...
      if (genMail?.id) {
        router.push(
          `${ApplicationURLS.USER_NOTICES_SEND_NOTICES_MAIL}/${genMail?.id}`
        );
      } else {
        setDisabledSaveBtn(false);
      }
            ...
            <div className={`mt-3 ${styles.sentContainer}`}>
              ...
              <CheckBox
                ...
                onChange={() => {
                  ...
                  if (
                    !sentCheckBox &&
                    QBCCNoticeTypesOnly.includes(data?.notice_type)
                  ) {
                      ...
                      {userMode !== "Onboarding" &&
                        ...
                        !QBCCNoticeTypesOnly.includes(data?.notice_type) && (
                          <span
                            ...
                            onClick={() =>
                              router.push(
                                `${ApplicationURLS.USER_NOTICES_SEND_NOTICES_MAIL}/view/${each?.id}`
                              )
                            }
                          >
                            {` - View`}
      ...
      btnConfig={
        <div className={styles.btnContainer}>
          ...
          <div className={styles.saveBtnsContianer}>
            {userMode !== "Onboarding" && !sentCheckBox && (
              <Button
    ...
    >
      <div className={styles.dataContainer}>
        <Form className={styles.formStyles} onSubmit={formik.handleSubmit}>
          ...
          <SignaturePadComponent disabled={false} /> */}
        ...
        <AppModal
          ...
          modalHeading={""}
          modalBodyContent={
            "Please upload the notice to the QBCC portal or post to them as per the template."
          }
          onConfirm={() => {
