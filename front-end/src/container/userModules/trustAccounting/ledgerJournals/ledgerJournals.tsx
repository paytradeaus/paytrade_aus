import React, { useCallback, useEffect, useRef, useState } from "react";
import {
  ArrowClockwise,
  FileEarmarkExcel,
  Printer,
} from "react-bootstrap-icons";
// import htmltopdf from "htmltopdf";
import { Col, Container, Form, Row } from "react-bootstrap";
import SearchableSelect from "@/components/SearchableSelect/SearchableSelect";
import CustomDatePicker from "@/components/customDatePicker/customDatePicker";
import { LedgerJournalTable } from "./ledgerJournalTable";
import {
  convertJsonToViewExcel,
  generatePDF,
  getLedgerJournalServices,
  LedgerJournalsType,
} from "./ledgerJournals.functions";
import sample from "../../../../../public/assets/payTradeLogo.png";
import html2pdf from "html2pdf.js";
// import { getProjectsLists } from "@/container/contracts/contracts.functions";
import { RowsPerPageInTable } from "@/common/constants";
import { FetchAllBankAccounts } from "../../bankTrustAccount/backTrustAccount.functions";
import { getCookie } from "cookies-next";
import { convertJsonToExcel, formatDate } from "@/common/commonFunctions";
import { DD_MM_YYYY } from "@/common/constants/general";
import { transactionsDateOptions } from "../../bankTrustAccount/bankTrustAccount.constant";
import styles from "./ledgerJournals.module.scss";
import { useSearchParams } from "next/navigation";
import TextField from "@/components/TextField/textField";
import { toast } from "@/app/Toaster";

interface HeaderBase {
  value: string;
  label: string;
}

interface MergedHeader extends HeaderBase {
  colspan: number;
  align: string;
}

type Header = {
  value: string;
  label: string;
  colspan?: number;
  align?: string;
};
const LedgerJournals = (props: any) => {
  const {
    isFromAdmin = false,
    isFromBankOverView = false,
    bankOverViewAcc = "",
  } = props;

  const BankAccId = useSearchParams().get("bank") || getCookie("bankId");
  const AdminCompanyId = getCookie("compId");

  const [selectedValue, setSelectedValue] = useState("");
  const [page, setPage] = useState(1);
  const [selectedAccountName, setSelectedAccountName] = useState<any>("");
  const [ledgerJournalListData, setLedgerJournalListData] = useState<
    LedgerJournalsType[]
  >([]);
  const [totalRows, setTotalRows] = useState(0);
  const [auditToggle, setAuditToggle] = useState(false);
  const [activityLogStartDate, setActivityLogStartDate] = useState(
    new Date(new Date().setHours(0, 0, 0, 0))
  );
  const [activityLogEndDate, setActivityLogEndDate] = useState(
    new Date(new Date().setHours(23, 59, 59, 999))
  );
  const [isCustomDate, setIsCustomDate] = useState(false);
  const [singleActivityDate, setSingleActivityDate] = useState<any>({
    value: "Last Month",
    label: "Last Month",
  });
  const [timeKey, setTimeKey] = useState(new Date().getTime());
  const [activityDate, setActivityDate] = useState("Last Month");
  const [selectedBankAccount, setSelectedBankAccount] = useState<any>(null);
  const [selectedRowsInGrid, setSelectedRowsInGrid] = useState([]);
  const [accountType, setAccountType] = useState<any>([]);
  const [debouncedSearch, setDebouncedSearch] = useState("");
  const [companyId, setCompanyId] = useState<number | null>(null);
  const [search, setSearch] = useState("");
  const [perPage, setPerPage] = useState(RowsPerPageInTable);
  const [loading, setLoading] = useState<boolean>(true);
  const [printDocumentData, setPrintDocumentData] = useState<any>([]);
  const [accountList, setAccountList] = useState<any>();
  const tableRef = useRef<HTMLTableElement>(null);
  const selectedCompanyId = Number(getCookie("companyId")) || 0;
  const handleAccountChange = (selectedOption: any) => {
    setSelectedAccountName(selectedOption);
    // Assuming the account type is stored in the selectedOption data.
    setAccountType(selectedOption?.data?.account_type);
  };
  const onInputChange = useCallback((e: { target: { value: string } }) => {
    const inputvalue = e?.target?.value?.trim()
      ? e?.target?.value
      : e?.target?.value?.trim();
    setSearch(inputvalue);
  }, []);
  const handleAuditToggle = () => {
    setAuditToggle(!auditToggle); // Toggling the audit view on/off
  };
  const resetFilters = () => {
    // Reset filtering criteria
    setSingleActivityDate({ value: "Last Month", label: "Last Month" });
    setIsCustomDate(false);
    setSelectedAccountName(accountList?.[0]); // Reset account selection to the first option

    // Reset date range to the previous month
    const now = new Date();
    const firstDayOfLastMonth = new Date(
      now.getFullYear(),
      now.getMonth() - 1,
      1
    );
    const lastDayOfLastMonth = new Date(now.getFullYear(), now.getMonth(), 0);

    setActivityLogStartDate(firstDayOfLastMonth);
    setActivityLogEndDate(lastDayOfLastMonth);

    // Trigger any additional reset logic (e.g., clearing grid selection)
    setSelectedRowsInGrid([]);
    setTimeKey(new Date().getTime());
  };
  const isAnyFilterActive =
    singleActivityDate.label !== "Last Month" ||
    (accountList?.length > 0 &&
      selectedAccountName?.value &&
      accountList?.[0].value !== selectedAccountName?.value);
  const handlePrintPDF = () => {
    const startDate = isCustomDate ? formatDate(activityLogStartDate) : null;
    const endDate = isCustomDate ? formatDate(activityLogEndDate) : null;

    const accountHeading = `${
      selectedAccountName?.label || "Default Account Heading"
    }`;

    const headerNames = auditToggle
      ? ["", "Account", "Audit ID", "Debit", "Credit"]
      : ["", "Account", "Debit", "Credit"];

    const checkIsAuditId = (data: any) => {
      return auditToggle
        ? data?.accounts?.map((account: any, index: number) => [
            index === 0 ? formatDate(data?.date) || "" : "",
            account?.account_name || "",
            account.audit_id || "",
            account?.debit || "",
            account?.credit || "",
          ])
        : data?.accounts?.map((account: any, index: number) => [
            index === 0 ? formatDate(data?.date) || "" : "",
            account?.account_name || "",
            account?.debit || "",
            account?.credit || "",
          ]) || [];
    };

    const formattedPDFData = () => {
      return ledgerJournalListData.flatMap((ledgerJournal) => {
        const rows = [
          // Header row
          headerNames.map((header, index) => {
            const isDateHeader = index === 0;
            const isRightAligned = header === "Debit" || header === "Credit";
            const style = `
      ${isDateHeader ? "text-align: left; width: 100px;" : ""}
      ${isRightAligned ? "text-align: right;" : "text-align: left;"}
      ${
        !isDateHeader
          ? "border-bottom: 1px solid black; border-top: 1px solid black;"
          : ""
      }
    `;
            return `<th style="${style}">${header}</th>`;
          }),

          // Data rows
          ...checkIsAuditId(ledgerJournal).map((rowData: any) =>
            rowData.map((data: any, index: number) => {
              let style = "text-align: left;";
              if (
                (auditToggle && index === 2) || // Journal description column when auditToggle is true
                (!auditToggle && index === 1) // Journal description column when auditToggle is false
              ) {
                style += `
                max-width: 200px;
                word-wrap: break-word;
                white-space: normal;
                overflow-wrap: break-word;
              `;
              } else if (
                (auditToggle && index >= 3 && index <= 4) ||
                (!auditToggle && index >= 2 && index <= 3)
              ) {
                // Check if value exceeds 7 digits
                const isLongText = data && data.toString().length > 7;
                style = `
                text-align: right; 
                max-width: 7em; 
                word-wrap: break-word; 
                white-space: normal; 
                overflow-wrap: break-word; 
                color: ${isLongText ? "inherit" : "inherit"};
              `;
              }
              return `<td style="${style}">${data}</td>`;
            })
          ),

          // Footer row
          [
            "",
            `${ledgerJournal?.journal_description || ""}`,
            ...(auditToggle ? [`${ledgerJournal?.audit_id || ""}`] : []),
            `${ledgerJournal?.total_debit || ""}`,
            `${ledgerJournal?.total_credit || ""}`,
          ].map((data, index) => {
            let style = "font-weight: bold;";
            if (index === 1) {
              // Journal description in footer
              style += `
              max-width: 250px;
              word-wrap: break-word;
              white-space: normal;
              overflow-wrap: break-word;
            `;
            } else if (
              (auditToggle && index >= 3 && index <= 4) ||
              (!auditToggle && index >= 2 && index <= 3)
            ) {
              // Check if value exceeds 7 digits
              const isLongText = data && data.toString().length > 7;
              style += `
              text-align: right; 
              max-width: 7em; 
              word-wrap: break-word; 
              white-space: normal; 
              overflow-wrap: break-word; 
              color: ${isLongText ? "inherit" : "inherit"};
            `;
            }
            return `<td style="${style}">${data}</td>`;
          }),
        ];

        return rows;
      });
    };

    // Restrict data to 500 records
    const pdfData = formattedPDFData();
    if (pdfData.length > 500) {
      toast.error(
        "PDF export failed. Please reduce the date period. Max 500 records for pdf export."
      );
      return;
    }

    const dateText = isCustomDate ? `From: ${startDate} To: ${endDate}` : "";
    const headerText = `${dateText ? `${dateText}` : ""}`;
    const tabHead = "Trust Accounting - Journals";
    const logoBase64 =
      ""; // Replace with your base64 logo string

    const paginatedData = paginateData(pdfData, 29, 35);

    generatePaginatedPDF(
      paginatedData,
      headerNames,
      accountHeading,
      headerText,
      tabHead,
      logoBase64
    );
  };

  // Paginate the data
  const paginateData = (
    data: any[],
    firstPageSize: number,
    subsequentPageSize: number
  ) => {
    const pages = [];
    // Handle the first page
    if (data.length > 0) {
      pages.push(data.slice(0, firstPageSize));
    }

    // Handle subsequent pages
    for (let i = firstPageSize; i < data.length; i += subsequentPageSize) {
      pages.push(data.slice(i, i + subsequentPageSize));
    }

    return pages;
  };

  // Generate the paginated PDF
  const generatePaginatedPDF = (
    pages: any[][],
    headerNames: string[],
    accountHeading: string,
    dateText: string,
    tabHead: string,
    logoBase64: string
  ) => {
    const headerContent = `
    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid black; padding-bottom: 10px;">
      <img src="data:image/png;base64,${logoBase64}" alt="Logo" style="width: 170px;" />
      <div style="font-weight: bold; font-size: 17px;">${tabHead}</div>
    </div>
    <div style="background-color: #E8E8E8; border-radius: 5px; text-align: left; padding-left: 5px; margin-top:12px; padding-top: 5px;">
      <p>${accountHeading}</p>
      <p style="padding-top:-30px; padding-bottom: 5px;">${dateText}</p>
    </div>
  `;

    let htmlContent = `<div>${headerContent}</div>`;
    pages.forEach((pageData, pageIndex) => {
      const pageRows = pageData
        .map(
          (item) => `
      <tr style="font-size: 12px; text-align: left;">
        ${item
          .map(
            (cell: string, index: number) =>
              `<td style="padding: 0px; ${
                index === 5 ? "text-align: right;" : "text-align: left;"
              }">${cell}</td>`
          )
          .join("")}
      </tr>
    `
        )
        .join("");

      const pageTable = `
      <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
        ${
          pageIndex === 0
            ? `<thead>
            <tr>
              ${headerNames
                .map(
                  (name) => `<th style="padding: 0px; text-align: left;"></th>`
                )
                .join("")}
            </tr>
          </thead>`
            : ""
        }
        <tbody>
          ${pageRows}
        </tbody>
      </table>
    `;

      htmlContent += `<div>${pageTable}</div>`;
      if (pageIndex < pages.length - 1) {
        htmlContent += `<div style="page-break-after: always;"></div>`;
      }
    });

    html2pdf()
      .from(htmlContent)
      .set({
        filename: "Account_Ledger_Report.pdf",
        html2canvas: { scale: 2 },
        margin: [7, 10, 0, 10],
        jsPDF: { orientation: "portrait" },
      })
      .toPdf()
      .get("pdf")
      .then((pdf: any) => {
        const pageCount = pdf.internal.getNumberOfPages();

        // Loop through each page to add pagination
        for (let i = 1; i <= pageCount; i++) {
          pdf.setPage(i);
          const fontSize = 8;
          const pageText = `Page ${i} of ${pageCount}`;
          pdf.setFontSize(fontSize);
          const textWidth =
            (pdf.getStringUnitWidth(pageText) * fontSize) /
            pdf.internal.scaleFactor;
          const pageWidth = pdf.internal.pageSize.getWidth();
          const pageHeight = pdf.internal.pageSize.getHeight();
          pdf.text(pageText, (pageWidth - textWidth) / 2, pageHeight - 10);
        }

        // Open the PDF in a new tab
        const pdfBlob = pdf.output("blob");
        const pdfUrl = URL.createObjectURL(pdfBlob);
        const newTab = window.open(pdfUrl, "_blank");

        // Trigger the download of the PDF
        pdf.save();
      })
      .catch((error: any) => {
        console.error("Error generating PDF:", error);
      });
  };

  const handleExportExcel = () => {
    if (printDocumentData?.length) {
      downloadExcel();
    }
  };

  const formatTextLine = (data: any[], keyName: string, isAmt = true) => {
    return data
      ?.map((v) =>
        v?.[keyName] === null ? "" : `${isAmt ? "$" : ""}${v?.[keyName]}`
      )
      ?.join("\n");
  };

  const downloadExcel = () => {
    // Define start and end dates based on custom date range
    const startDate = isCustomDate ? formatDate(activityLogStartDate) : null;
    const endDate = isCustomDate ? formatDate(activityLogEndDate) : null;

    // Determine the account heading based on accountType
    const accountHeading =
      accountType === "Project Trust Account"
        ? "Project Trust Account"
        : "Retention Trust Account";

    // const headerNames = auditToggle
    //   ? ["", "Account", "Audit ID", "Debit", "Credit"]
    //   : ["", "Account", "Debit", "Credit"];

    const checkIsAuditId = (data: any) => {
      const fieldName = { isAudit: [], notAudit: [] };
      return auditToggle
        ? data?.accounts?.map((account: any, index: number) => [
            index === 0 ? formatDate(data?.date) || "" : "", // Date or empty string
            account?.account_name || "", // Account Name
            auditToggle ? account.audit_id || "" : "", // Include Audit ID if toggled
            account?.debit || "", // Debit
            account?.credit || "", // Credit
          ])
        : data?.accounts?.map((account: any, index: number) => [
            index === 0 ? formatDate(data?.date) || "" : "", // Date or empty string
            account?.account_name || "", // Account Name
            auditToggle ? "" : null,
            account?.debit || "", // Debit
            account?.credit || "", // Credit
          ]) || [];
    };

    // Function to format Excel data
    const formattedExcelData = () => {
      return ledgerJournalListData.flatMap((ledgerJournal: any) => {
        const accounts = ledgerJournal?.accounts || [];
        const description = ledgerJournal.journal_description || "";
        const totalDebit = ledgerJournal.total_debit || "";
        const totalCredit = ledgerJournal.total_credit || "";
        return accounts
          .flatMap((account: any, index: number) => {
            const isLastIndex = index === accounts.length - 1;
            return [
              {
                date:
                  index === 0
                    ? ledgerJournal?.date
                      ? formatDate(ledgerJournal?.date)
                      : null
                    : null,
                account: account.account_name || "",
                audit: auditToggle ? account.audit_id || "" : "",
                debit: account.debit || "",
                credit: account.credit || "",
                description: "", // Empty description for account rows
              },
              // Add an extra row for the description below the last account row
              isLastIndex
                ? {
                    date: "",
                    account: description, // Place description in the account column
                    debit: totalDebit, // Place total_debit in the debit column
                    credit: totalCredit, // Place total_credit in the credit column
                    description: "", // Empty description
                  }
                : null,
            ];
          })
          .filter(Boolean); // Remove null values
      });
    };

    // Define column names for the table
    const columnNames = auditToggle
      ? [
          { value: "date", label: "Date" },
          { value: "account", label: "Account" },
          { value: "audit", label: "Audit Id" },
          { value: "debit", label: "Debit" },
          { value: "credit", label: "Credit" },
        ]
      : [
          { value: "date", label: "Date" },
          { value: "account", label: "Account" },
          { value: "debit", label: "Debit" },
          { value: "credit", label: "Credit" },
        ];

    // Define the header row with the dynamic heading based on the account type
    const headers: Header[] = [
      {
        value: isCustomDate
          ? `Journal Report - ${accountHeading} From [${startDate}] To [${endDate}]`
          : `Journal Report - ${accountHeading}`,
        colspan: columnNames.length,
        align: "center",
        label: "",
      },
    ];

    // Convert JSON data to Excel with additional header
    convertJsonToViewExcel(
      formattedExcelData(),
      "current ledger journal list",
      columnNames,
      headers // Pass the headers with the dynamic heading
    );
  };

  useEffect(() => {
    (async () => {
      const response = await FetchAllBankAccounts({
        company_id:
          isFromAdmin && AdminCompanyId
            ? Number(AdminCompanyId)
            : selectedCompanyId,
        page: 1,
        items_per_page: null,
        is_alphabetical_order: true,
        account_type: AdminCompanyId
          ? null
          : "Project Trust Account, Retention Trust Account",
      });
      // const [projectList] = await Promise.all([
      //   getProjectsLists(selectedCompanyId),
      // ]);
      if (
        response?.extendedBankAccounts &&
        response?.extendedBankAccounts.length > 0
      ) {
        const customOption = response.extendedBankAccounts.map((data: any) => ({
          label: data.account_name,
          value: data.bank_account_id.toString(),
          data: {
            account_type: data.account_type, // Ensure this is included
            ...data,
          },
        }));
        setAccountList(customOption || []);
        // Check if BankAccId is present and set the corresponding account
        const selectedAccount = customOption.find((option: any) => {
          if (isFromBankOverView && bankOverViewAcc) {
            return option.value === bankOverViewAcc;
          } else {
            return option.value === BankAccId;
          }
        });
        if (selectedAccount) {
          setSelectedAccountName(selectedAccount);
        } else {
          setSelectedAccountName(customOption?.[0]);
        }
      }
    })();
  }, [AdminCompanyId, selectedCompanyId, BankAccId]);
  useEffect(() => {
    const storedCompanyId = localStorage.getItem("companyId");

    if (isFromAdmin && AdminCompanyId) {
      setCompanyId(Number(AdminCompanyId));
    } else if (storedCompanyId) {
      setCompanyId(parseInt(storedCompanyId, 10));
    }
  }, [AdminCompanyId]);

  useEffect(() => {
    if (companyId !== null && selectedAccountName) {
      fetchData(page, perPage);
    }
  }, [
    companyId,
    selectedValue,
    selectedAccountName,
    singleActivityDate,
    selectedBankAccount,
    search,
    activityLogStartDate,
    activityDate,
    activityLogEndDate,
    debouncedSearch,
    AdminCompanyId,
  ]);

  const handleActivityChange = (selectedValue: any) => {
    // setSelectedRowsInGrid([]);
    setTimeKey(new Date().getTime());
    setSingleActivityDate(selectedValue);
    setIsCustomDate(selectedValue.value === "Custom");
    setActivityDate(selectedValue.value);
  };

  const fetchData = async (page: number, rowsPerPage: number) => {
    setLoading(true);
    try {
      const response = await getLedgerJournalServices({
        company_id:
          isFromAdmin && AdminCompanyId ? Number(AdminCompanyId) : companyId,
        page_number: page,
        page_size: rowsPerPage,
        search: search ? search : null,
        bank_account_id: selectedAccountName?.value
          ? Number(selectedAccountName?.value)
          : "",
        date_filter: singleActivityDate?.value || null,
        start_date: isCustomDate ? activityLogStartDate : null,
        end_date: isCustomDate ? activityLogEndDate : null,
      });

      if (response) {
        const newdata = response?.ledger_journals_list?.map((prev: any) => ({
          ...prev,
        }));

        setLedgerJournalListData(newdata || []);
        setTotalRows(response?.total_count || 0);
        setPerPage(rowsPerPage);

        const printDataObjCreation = response?.ledger_journals_list?.map(
          (ledgerJournal: any) => ({
            date: ledgerJournal?.date,
            first_account_name: ledgerJournal.accounts[0]?.account_name || "",
            second_account_name: ledgerJournal.accounts[1]?.account_name || "",
            description: ledgerJournal?.journal_description || "",
            first_debit: ledgerJournal.accounts[0]?.debit || "",
            second_debit: ledgerJournal.accounts[1]?.debit || "",
            third_debit: ledgerJournal?.total_debit || "",
            first_credit: ledgerJournal.accounts[0]?.credit || "",
            second_credit: ledgerJournal.accounts[1]?.credit || "",
            third_credit: ledgerJournal?.total_credit || "",
          })
        );

        setPrintDocumentData(printDataObjCreation);
      } else {
        setLedgerJournalListData([]);
        setTotalRows(0);
      }
    } catch (error) {
      console.log("error", error);
    } finally {
      setLoading(false);
    }
  };

  return (
    <>
      <div
        style={{
          width: "100%",
          display: "flex",
          // alignItems: "center",
          // justifyContent: "center",
          alignContent: "space-between",
          textAlign: "left",
          gap: "20px",
        }}
      >
        <div style={{ width: "8%" }}>
          <span>
            <Form.Check
              type="switch"
              id="audit-switch"
              label={auditToggle ? "Audit View" : "Audit View"}
              checked={auditToggle}
              onChange={handleAuditToggle} // Toggle functionality
              className={styles.switch}
            />
          </span>
        </div>
        {auditToggle && (
          <div style={{ width: "15%" }}>
            <TextField
              placeholder="search using claim id, payment id"
              value={search}
              onChange={onInputChange}
              type="text"
              // autoFocus
              className={styles.textFieldStyles}
            />
          </div>
        )}
        <div style={{ width: "15%" }}>
          <SearchableSelect
            options={accountList}
            onChange={handleAccountChange} // Updated to use handleAccountChange
            placeholder="Account Name"
            selectedData={selectedAccountName}
            disabled={isFromBankOverView}
          />
        </div>

        <div style={{ width: "15%" }}>
          <SearchableSelect
            options={transactionsDateOptions}
            onChange={handleActivityChange}
            placeholder="Dates"
            selectedData={singleActivityDate}
          />
        </div>

        <div className={styles.datepickerCustom}>
          {isCustomDate && (
            <>
              <CustomDatePicker
                showIcon={true}
                toggleCalendarOnIconClick
                placeholderText="&nbsp;From date"
                selected={activityLogStartDate}
                value={activityLogStartDate}
                onChange={(selectedDate: string) => {
                  let fromDate = new Date(
                    new Date(selectedDate).setHours(0, 0, 0, 0)
                  );
                  if (fromDate > activityLogEndDate) {
                    // setSelectedRowsInGrid([]);
                    setTimeKey(new Date().getTime());
                    setActivityLogStartDate(fromDate);
                    setActivityLogEndDate(new Date(selectedDate));
                  } else {
                    // setSelectedRowsInGrid([]);
                    setTimeKey(new Date().getTime());
                    setActivityLogStartDate(fromDate);
                  }
                }}
                disabled={false}
                format={DD_MM_YYYY}
                className={styles.DatePickerCustomStyles}
              />

              <CustomDatePicker
                showIcon={true}
                toggleCalendarOnIconClick
                placeholderText="&nbsp;To date"
                selected={activityLogEndDate}
                value={activityLogEndDate}
                onChange={(selectedDate: string) => {
                  let toDate = new Date(selectedDate);
                  if (toDate < activityLogStartDate) {
                    return;
                  } else {
                    // setSelectedRowsInGrid([]);
                    setTimeKey(new Date().getTime());
                    setActivityLogEndDate(toDate);
                  }
                }}
                disabled={false}
                format={DD_MM_YYYY}
                className={styles.DatePickerCustomStyles}
              />
            </>
          )}
        </div>
        {isAnyFilterActive && (
          <div style={{ width: "21%" }}>
            <button onClick={resetFilters} className={styles.resetButton}>
              <ArrowClockwise /> Reset Filters
            </button>
          </div>
        )}

        {auditToggle && <div style={{ width: "0%" }}></div>}

        <div style={{ width: "62%" }}>
          <div className={styles.headerIconCon}>
            <span
              className={styles.iconStyles}
              onClick={() => {
                if (printDocumentData?.length) {
                  handlePrintPDF();
                }
              }}
              title="Print PDF"
            >
              <Printer />
            </span>

            <span
              className={styles.iconStyles}
              onClick={handleExportExcel}
              title="Export to Excel"
            >
              <FileEarmarkExcel />
            </span>
          </div>
        </div>
      </div>

      <div className={styles.mainContainer}>
        {loading ? (
          <p className={styles.noRecordStyle}>Loading...</p>
        ) : ledgerJournalListData.length === 0 ? (
          <p className={styles.noRecordStyle}>
            There are no records to display
          </p>
        ) : (
          <LedgerJournalTable
            data={ledgerJournalListData}
            auditToggle={auditToggle}
          />
        )}
      </div>
    </>
  );
};

export default LedgerJournals;
