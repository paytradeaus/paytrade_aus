...
// import { showErrorToast } from "@/path/to/toastHOC"; // Import showErrorToast from the HOC
...
export async function triggerForgotPasswordEmail(inputData: Object) {
  try {
    ...
    if (response?.data?.sendVerificationCode?.status === ERROR) {
      showErrorToast(response?.data?.sendVerificationCode?.message);
      return false;
    }
  } catch (error: any) {
    // Handle errors
...
export async function verifyForgotPasswordCode(inputData: Object) {
  try {
    ...
    if (response?.data?.verifyCode?.status === ERROR) {
      console.log("msg: ", response?.data?.verifyCode?.message);
      showErrorToast("This code is invalid");
      return null;
    }
  } catch (error: any) {
    // Handle errors
...
export const switchModeOfAnUser = async (
  ...
  try {
    ...
    if (response?.data?.switchModeOfAnUser?.status === SUCCESS) {
      showSuccessToast(response?.data?.switchModeOfAnUser?.message);
