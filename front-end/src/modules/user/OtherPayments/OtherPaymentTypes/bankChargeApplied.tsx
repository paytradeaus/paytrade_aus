...
const BankChargeAppliedForm = (props: any) => {
  ...
  useEffect(() => {
    if (trustAccountList && data?.payment_id) {
      ...
      let accountList = trustAccountList?.find(
        ...
        label: data?.payment_from_account?.toString(),
        value: data?.payment_from_account?.toString(),
      };
      setSelectedValue(accountList);
...
export default BankChargeAppliedForm;
